...
const PROMPT = "$";
const BOOT_COMMAND = "aestrodom/out";
const PASS_KEY = "aestrodom.pass";

const HELP: Line[] = [
  { text: "aestrodom/out                chain-wide coverage             free", tone: "ink" },
  { text: "aestrodom/market <SYMBOL>    one market in detail            free", tone: "ink" },
  { text: "aestrodom/exit <SYMBOL> <USD> can this size get out?          free · daily snapshot", tone: "ink" },
  { text: "aestrodom/proof <SYMBOL>     the command to check it yourself  free", tone: "ink" },
  { text: "" },
  { text: "aestrodom/live <SYMBOL> <USD> the same question, this block   1 USDG · 30 days", tone: "amber" },
  { text: "aestrodom/pay <TX HASH>      claim your payment (wallet signs it)", tone: "amber" },
  { text: "aestrodom/pass               what your pass is worth", tone: "amber" },
  { text: "" },
...
function chainWide(): Line[] {
  ...
  return [
    ...
    { text: "type  aestrodom/exit USDe 1000000  to ask about a size", tone: "dim" },
  ];
}
...
function exitAnswer(symbol: string, sizeRaw: string): Line[] {
  ...
  if (size === null) {
    return [{ text: `"${sizeRaw}" is not a size. try: aestrodom/exit USDe 1000000`, tone: "ember" }];
  }
  ...
  lines.push({
    text: `for this block instead:  aestrodom/live ${market.symbol} ${size}`,
    tone: "dim",
  });
  return lines;
}
  ...
  if (!signature) {
    ...
    try {
      ...
      signature = (await window.ethereum.request({
        ...
        params: [`aestrodom-pass:${hash.toLowerCase()}`, accounts[0]],
      })) as string;
    } catch {
      return [
        { text: "the wallet declined to sign.", tone: "ember" },
        { text: "signing costs nothing and moves nothing — it only proves the", tone: "dim" },
        { text: "payment is yours. run aestrodom/pay again to retry, or:", tone: "dim" },
...
function passStatus(session: Session): Line[] {
  if (!session.pass) {
    return [
      ...
      { text: "run  aestrodom/live USDe 1000000  to see what one costs.", tone: "dim" },
    ];
  }
  ...
  return [
    ...
    { text: "in this browser. we store nothing about you. aestrodom/forget drops it.", tone: "dim" },
  ];
}
...
function run(input: string, session: Session): Line[] | Promise<Line[]> {
  ...
  switch (command) {
    ...
    case "hatch/out":
      return chainWide();
    case "aestrodom/market":
    case "lingo/market":
    case "hatch/market":
      return args[0] ? marketDetail(args[0]) : [{ text: "usage: aestrodom/market <SYMBOL>", tone: "dim" }];
    case "aestrodom/exit":
    case "lingo/exit":
    case "hatch/exit": {
      if (!args[0] || !args[1]) return [{ text: "usage: aestrodom/exit <SYMBOL> <USD>", tone: "dim" }];
      const { symbol, sizeRaw } = symbolAndSize(args[0], args[1]);
      return exitAnswer(symbol, sizeRaw);
    }
    case "aestrodom/proof":
    case "lingo/proof":
    case "hatch/proof":
      return args[0] ? proof(args[0]) : [{ text: "usage: aestrodom/proof <SYMBOL>", tone: "dim" }];
    case "aestrodom/live":
    case "lingo/live":
    case "hatch/live": {
      if (!args[0] || !args[1]) return [{ text: "usage: aestrodom/live <SYMBOL> <USD>", tone: "dim" }];
      const { symbol, sizeRaw } = symbolAndSize(args[0], args[1]);
      const size = parseSize(sizeRaw);
      if (size === null) {
        return [{ text: `"${sizeRaw}" is not a size. try: aestrodom/live USDe 1000000`, tone: "ember" }];
      }
    ...
    case "hatch/pay":
      return args[0]
        ...
        : [{ text: "usage: aestrodom/pay <TX HASH>  (or: aestrodom/pay <TX HASH> <SIGNATURE>)", tone: "dim" }];
    case "aestrodom/pass":
    case "lingo/pass":
    case "hatch/pass":
      return passStatus(session);
    case "aestrodom/forget":
    case "lingo/forget":
    case "hatch/forget":
      forgetPass(session);
  ...
  return (
    <div className={styles.console} onClick={() => inputRef.current?.focus()}>
      <div className={styles.chrome}>
        <span className={styles.label}>AESTRODOM CONSOLE</span>
        ...
        {booted && (
          <form onSubmit={submit} className={styles.row}>
            ...
            <input
              ...
              placeholder="aestrodom/live USDe 1000000"
            />
